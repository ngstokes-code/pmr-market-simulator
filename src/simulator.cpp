...
Simulator::Simulator(SimConfig cfg) : cfg_(std::move(cfg)), rng_(cfg_.seed) {
    auto symbols = cfg_.symbol_list.empty() ? default_symbols() : cfg_.symbol_list;
    for (auto& s : symbols) {
...
double Simulator::draw_price(const std::string& s, uint64_t i_event) {
    ...
    if (cfg_.drift_ampl > 0.0 && cfg_.drift_period > 0) {
        double phase = (double)(i_event % cfg_.drift_period) / (double)cfg_.drift_period;
...
void Simulator::run() {
    ...
    for (uint64_t i = 0; i < cfg_.total_events; ++i) {
        ...
        if (is_add || next_order_id_ <= 10) {
            ...
            Event e;
            e.ts_ns = o.ts_ns; e.symbol = s; e.price = o.price; e.qty = o.qty; e.side = o.side;
            if (matched > 0) {
                e.type = EventType::TRADE; e.price = trade_px; e.qty = matched;
                emit_event(e); ++trades;
            } else {
            ...
            if (bb && ba) st.mid = (*bb + *ba) * 0.5;
            else if (bb) st.mid = *bb;
            else if (ba) st.mid = *ba;
        } else {
